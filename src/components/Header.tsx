    ...
    <header className="min-h-[128px] px-4 md:px-8 flex flex-col justify-center">
      <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
          <button onClick={onMenuClick} className="lg:hidden p-2 -mr-2 text-[#0a1e3b]">
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
          </button>
          
          <div className="flex items-center gap-3">
            <div className="flex flex-col text-right">
              <span className="text-sm font-medium text-[#0a1e3b] line-clamp-1">مساء الخير MOHAMMAD ALAMIN GOLAP</span>
        ...
        <div className="flex items-center gap-2 order-2 sm:order-3 w-full sm:w-auto justify-end">
          <button className="hidden sm:flex bg-[#e8f5e9] text-[#2e7d32] px-4 py-2 rounded-full text-[10px] md:text-xs font-bold items-center gap-2 border border-[#8BC34A]/30 hover:bg-[#c8e6c9] transition-all whitespace-nowrap">
            <svg className="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            العودة للموقع
          </button>

          <button className="sm:hidden h-9 px-3 bg-[#e8f5e9] text-[#2e7d32] rounded-full text-[10px] font-bold border border-[#8BC34A]/30">العودة</button>
      ...
      <div className="hidden sm:block text-sm text-[#0070f3] font-bold mt-2 mr-14">كيف يمكننا مساعدتك اليوم؟</div>
    </header>
  );
};
